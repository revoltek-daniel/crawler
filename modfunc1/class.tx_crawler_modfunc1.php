...
class tx_crawler_modfunc1 extends \TYPO3\CMS\Backend\Module\AbstractFunctionModule
{
     ...
     */
    var $incomingConfigurationSelection = [];
    ...
    /**
     * @var tx_crawler_domain_process_manager
     */
    ...
    private function drawURLs()
    {
        ...
        if ($code) {
            ...
            if ( ! $this->submitCrawlUrls) {
                ...
                $output .= $GLOBALS['LANG']->sL('LLL:EXT:crawler/modfunc1/locallang.xml:labels.curtime') . ': ' . date('H:i:s',
                        time()) . '<br />';
                $output .= '<br />
     ...
     */
    private function drawURLs_cfgSelectors()
    {
        $cell = [];
        // depth
        ...
        $availableConfigurations = $this->crawlerObj->getConfigurationsForBranch($this->pObj->id,
            $this->pObj->MOD_SETTINGS['depth'] ? $this->pObj->MOD_SETTINGS['depth'] : 0);

        // Configurations
        $cell[] = $this->selectorBox(
            empty($availableConfigurations) ? [] : array_combine($availableConfigurations,
     ...
     */
    private function drawURLs_printTableHeader()
    {
    ...
    private function drawLog()
    {
        ...
        if (\TYPO3\CMS\Core\Utility\GeneralUtility::_GP('qid_details')) {
            ...
            // Get entry record:
            list($q_entry) = $GLOBALS['TYPO3_DB']->exec_SELECTgetRows('*', 'tx_crawler_queue',
                'qid=' . intval(\TYPO3\CMS\Core\Utility\GeneralUtility::_GP('qid_details')));
            ...
            if ($this->pObj->id || $showSetId) {
                if ($this->pObj->id) {
                    ...
                    foreach ($tree->tree as $data) {
                        $code .= $this->drawLog_addRows(
                            $data['row'],
                            $data['HTML'] . \TYPO3\CMS\Backend\Utility\BackendUtility::getRecordTitle('pages',
                                $data['row'], true),
                ...
                if ($code) {
                    ...
                    $output .= '
						...
						<input type="hidden" value="' . $showSetId . '" name="setID" />
						<br />
						' . $GLOBALS['LANG']->sL('LLL:EXT:crawler/modfunc1/locallang.xml:labels.curtime') . ': ' . date('H:i:s',
                            time()) . '
						<br /><br />
     ...
     */
    private function drawLog_addRows($pageRow_setId, $titleString, $itemsPerPage = 10)
    {
        ...
        if (is_array($pageRow_setId)) {
            $res = $this->crawlerObj->getLogEntriesForPageId($pageRow_setId['uid'],
                $this->pObj->MOD_SETTINGS['log_display'], $doFlush, $doFullFlush, intval($itemsPerPage));
        } else {
            $res = $this->crawlerObj->getLogEntriesForSetId($pageRow_setId, $this->pObj->MOD_SETTINGS['log_display'],
                $doFlush, $doFullFlush, intval($itemsPerPage));
        }
            ...
            foreach ($res as $kk => $vv) {
                ...
                $content .= '
					<tr class="bgColor' . ($c % 2 ? '-20' : '-10') . '">
						' . $titleClm . '
						<td><a href="' . $this->getModuleUrl([
                        'qid_details' => $vv['qid'],
                        'setID' => $setId
                    ]) . '">' . htmlspecialchars($vv['qid']) . '</a></td>
						<td><a href="' . $this->getModuleUrl([
                        'qid_read' => $vv['qid'],
                        'setID' => $setId
                    ]) . '"><img src="' . $refreshIcon . '" width="14" hspace="1" vspace="2" height="14" border="0" title="' . htmlspecialchars('Read') . '" alt="" /></a></td>';
     ...
     * @return array
     */
    private function getResFeVars($row)
    {
        $feVars = [];
     ...
     * @return    string        Table header
     */
    private function drawLog_printTableHeader()
    {
    ...
    private function getResStatus($vv)
    {
        if ($vv['result_data']) {
            ...
            if (is_array($requestResult)) {
                ...
                $resStatus = 'Error: ' . substr(preg_replace('/\s+/', ' ', strip_tags($requestContent['content'])), 0,
                        10000) . '...';
            }
        } else {
            $resStatus = '-';
    ...
    protected function drawProcessOverviewAction()
    {
        ...
        $listView->setMaxActiveProcessCount(\TYPO3\CMS\Core\Utility\MathUtility::forceIntegerInRange($this->extensionSettings['processLimit'],
            1, 99, 1));
    ...
    protected function makeCrawlerProcessableChecks()
    {
        ...
        if ($exitCode > 0) {
            $this->addErrorMessage(sprintf($LANG->sL('LLL:EXT:crawler/modfunc1/locallang.xml:message.phpBinaryNotFound'),
                htmlspecialchars($this->extensionSettings['phpPath'])));
        }
    }
    ...
    {
        $isAvailable = false;
        $userArray   = \TYPO3\CMS\Backend\Utility\BackendUtility::getRecordsByField('be_users', 'username',
            '_cli_crawler');
     ...
     */
    private function selectorBox($optArray, $name, $value, $multiple)
    {

        $options = [];
        foreach ($optArray as $key => $val) {
            $options[] = '
				<option value="' . htmlspecialchars($key) . '"' . (( ! $multiple && ! strcmp($value,
                        $key)) || ($multiple && in_array($key,
                        (array)$value)) ? ' selected="selected"' : '') . '>' . htmlspecialchars($val) . '</option>';
        }

        $output = '<select name="' . htmlspecialchars($name . ($multiple ? '[]' : '')) . '"' . ($multiple ? ' multiple="multiple" size="' . count($options) . '"' : '') . '>' . implode('',
                $options) . '</select>';
     ...
     * @return    void
     */
    protected function runRefreshHooks()
    {
    ...
    protected function getModuleUrl(array $urlParameters = [])
    {
        ...
        return \TYPO3\CMS\Backend\Utility\BackendUtility::getModuleUrl(\TYPO3\CMS\Core\Utility\GeneralUtility::_GP('M'),
            $urlParameters);
    }
}
